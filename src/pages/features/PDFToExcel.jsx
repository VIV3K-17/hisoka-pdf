...
export const PDFToExcel = () => {
    ...
    return (
        <FeatureLayout title="PDF to Excel" description="Pull data straight from PDFs into Excel spreadsheets in a few short seconds" icon={Sheet} gradient="from-teal-500 to-cyan-500">
            <div className="space-y-6">
                <div className={`p-4 rounded-xl border flex items-start gap-3 ${isDarkMode ? 'bg-blue-500/10 border-blue-500/30' : 'bg-blue-50 border-blue-200'}`}>
                    ...
                    <div>
                        ...
                        <p className={`text-xs mt-1 ${isDarkMode ? 'text-blue-400' : 'text-blue-700'}`}>Connect your API endpoint to enable PDF to Excel conversion.</p>
                    </div>
                </div>
                {!file ? (
                    <div className={`border-2 border-dashed rounded-2xl p-8 text-center transition-all ${isDarkMode ? 'border-white/20 hover:border-white/40 bg-white/5' : 'border-gray-300 hover:border-gray-400 bg-gray-50'}`}>
                        ...
                        <label htmlFor="pdf-upload" className="inline-block px-6 py-3 bg-gradient-to-r from-teal-500 to-cyan-500 text-white rounded-lg font-medium cursor-pointer hover:opacity-90 transition-opacity">Select PDF</label>
                    </div>
                ) : (
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                {['xlsx', 'xls'].map((fmt) => (
                                    <button key={fmt} onClick={() => setFormat(fmt)} className={`p-4 rounded-xl border-2 transition-all ${format === fmt ? 'border-teal-500 bg-teal-500/10' : isDarkMode ? 'border-white/10 hover:border-white/20' : 'border-gray-200 hover:border-gray-300'}`}>
                                        <p className={`font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>.{fmt.toUpperCase()}</p>
                                    </button>
                                ))}
                            </div>
                        </div>
                        <button onClick={handleConvert} disabled={isProcessing} className="w-full py-4 bg-gradient-to-r from-teal-500 to-cyan-500 text-white rounded-xl font-semibold text-lg hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center justify-center gap-2">
                            {isProcessing ? <><Loader2 size={24} className="animate-spin" />Converting...</> : <><Download size={24} />Convert to Excel</>}
                        </button>
                    </motion.div>
                )}
            </div>
